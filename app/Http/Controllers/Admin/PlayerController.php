...
class PlayerController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function update_money(Request $request,$id){
        ...
        if($request->type == 'tang'){
            $player->money = (string)($player->money + $request->money_up_down);
            prepare_bill(
                $player->id,
                "<p>*Tăng tiền</p>
                <p>- Số tiền tăng: ". number_format($request->money_up_down) ."</p>
                <p>- Trạng thái: Tăng tiền</p>
                ",
                auth()->user()->id);
        }
        if($request->type == 'tru'){
            if($request->money_up_down > $player->money){
                return redirect()->back()->with([
                    'message'    => "Không đủ số dư để trừ tiền ",
                    'alert-type' => 'warning',
                ]);
            }
            else{
                ...
                prepare_bill(
                    $player->id,
                    "<p>*Trừ tiền</p>
                    <p>- Số tiền trừ: ". number_format($request->money_up_down) ."</p>
                    <p>- Trạng thái: Trừ tiền</p>
                    ",
                    auth()->user()->id);
            }  
        } 
